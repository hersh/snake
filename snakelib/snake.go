...
func (snake *Snake) Update(_map *Map) UpdateResult {
	ch := _map.GetCell(snake.NextPos())
	if isStopper(ch) {
		if isStopper(_map.GetCell(snake.HeadPos().PlusDir(Down, 1))) &&
			isStopper(_map.GetCell(snake.HeadPos().PlusDir(Up, 1))) &&
			isStopper(_map.GetCell(snake.HeadPos().PlusDir(Left, 1))) &&
			isStopper(_map.GetCell(snake.HeadPos().PlusDir(Right, 1))) {
